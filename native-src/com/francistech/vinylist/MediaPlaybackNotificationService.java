...
public class MediaPlaybackNotificationService extends Service {
    ...
    // Safety cap on the wake lock below -- if something goes wrong and we
    // never get a chance to release it (process death mid-playback, a
    // missed edge case, etc.), Android force-releases it after this long
    // instead of it silently draining the battery forever. Renewed on every
    // playing-state update, so in normal use it never actually gets close
    // to this while a track is genuinely still playing.
    ...
    public void onCreate() {
        ...
        // Partial wake lock, held only while a track is actually playing (see
        // updateSessionState()). Without this, once the screen turns off
        // Android is free to let the CPU sleep between audio buffer
        // callbacks, which can stall this process for long enough that
        // Doze/App Standby treats it as idle -- observed as the notification
        // and its controls going stale or vanishing after a while of
        // background playback, only coming back once the app was reopened.
        // setReferenceCounted(false) makes acquire()/release() idempotent so
        // repeated state updates while already playing don't need to be
        // paired up perfectly.
        PowerManager pm = (PowerManager) getSystemService(Context.POWER_SERVICE);
        wakeLock = pm != null
            ? pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "Vinylist:PlaybackWakeLock")
            : null;
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        // Apply metadata/state changes carried by *this* Intent before the
        // notification below gets built -- not after. This used to run
        // after startForegroundCompat(), so the notification always
        // rendered with the *previous* state and only caught up on the
        // following Intent. In practice: tapping Play sends ACTION_PLAY
        // (starts the service, builds the notification with the old,
        // pre-play state) and JS then sends a separate ACTION_UPDATE_STATE
        // once playback actually starts -- but the notification's own
        // Play/Pause icon didn't flip to Pause until it happened to update
        // for some *other* reason later, since applyStateExtras() ran after
        // the notification had already been built and posted. Applying the
        // extras first means the very first notification for an update
        // Intent already reflects it.
        ...
        // Must run next, unconditionally -- including for ACTION_STOP.
        // MediaControlPlugin#startServiceCompat always launches this
        // service via startForegroundService() on API 26+, so Android
        // requires startForeground() to be called within a few seconds no
        // matter which action we're handling. The STOP branch used to
        // return early (via stopSelfSafely()) before this ever ran, which
        // broke that contract and crashed the app with
        // ForegroundServiceDidNotStartInTimeException -- reachable on cold
        // launch, before the user ever presses Play, whenever the JS side
        // sends a STOP intent for a session that never actually started.
        ...
        if (ACTION_PLAY.equals(action)) {
            ...
            stopSelfSafely(); // safe now: startForegroundCompat() already ran above
            return START_NOT_STICKY;
        }

        // If playback is actually paused we immediately relax the
        ...
        // STICKY (not NOT_STICKY) for every path that reaches here -- i.e.
        // every path except the explicit ACTION_STOP one above, which
        // already returned early. If the OS kills this process outright
        // during background playback (low memory, an OEM's own "battery
        // saver" force-stop, etc.), this asks Android to recreate the
        // service afterwards with a null Intent instead of leaving it dead
        // until the person happens to reopen the app. A bare restart can't
        // fully resurrect playback by itself -- title/artist/isPlaying live
        // on this instance and don't survive a process death -- but it
        // means there's a notification to tap (opening the app) rather than
        // nothing at all, and the JS-side resume resync above takes it from
        // there the moment the app is actually reopened.
        return START_STICKY;
    }
    ...
    private void applyWakeLock() {
        ...
        if (isPlaying) {
            // Re-acquiring an already-held, non-reference-counted lock just
            // renews its timeout -- this runs on every state ping (including
            // the JS heartbeat every 15s while playing), so the lock stays
            // held continuously through real playback and only ever expires
            // on its own if those pings actually stop coming in.
    ...
    private Notification buildNotification() {
        NotificationCompat.Builder builder = new NotificationCompat.Builder(this, CHANNEL_ID)
            ...
            // Deliberately NOT tied to isPlaying. This used to be
            // .setOngoing(isPlaying), which made the notification a normal,
            // swipeable one the instant isPlaying reported false for any
            // reason -- including a brief, transient blip during a track
            // crossfade/gapless swap, not just a genuine user-initiated
            // pause. Once swipeable, a stray notification-shade interaction,
            // an OEM's aggressive "clean up notifications" routine, or
            // Android 13+'s own auto-hide-for-paused-media behavior could
            // clear it entirely -- while playback (driven independently by
            // the WebView's own audio pipeline) just kept going right
            // through it, only for the notification to reappear once the
            // app was reopened and sent a fresh state update. Keeping this
            // pinned for the life of the session and only ever removing it
            // via the explicit Stop path (deleteIntent below, which calls
            // stopSelfSafely()'s nm.cancel()) closes that whole window.
            .setOngoing(true)
            .setShowWhen(false)
