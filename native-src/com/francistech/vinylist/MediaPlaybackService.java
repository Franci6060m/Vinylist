...
public class MediaPlaybackService extends Service {
    ...
    // Vinylist's brand accent (matches the app's dark theme); used as the
    // notification's colorized-card fallback. Tweak to taste.
    private static final int ACCENT_COLOR = 0xFF6C4FE0;
    ...
    // Lets MainActivity know whether it's safe/necessary to keep the WebView's
    // JS running while the Activity itself is backgrounded (see MainActivity's
    // onPause/onStop override). True from onCreate until onDestroy.
    ...
    private Notification buildNotification() {
        ...
        // Same target as deletePI, bound to its own request code so it gets a
        // distinct PendingIntent -- this is the tappable "X" action button
        // (as opposed to deletePI, which only fires on swipe-to-dismiss).
        PendingIntent closePI = PendingIntent.getService(
            this, 1, new Intent(this, MediaPlaybackService.class).setAction(ACTION_STOP_SELF), piFlags
        );
        ...
        NotificationCompat.Builder builder = new NotificationCompat.Builder(this, CHANNEL_ID)
            ...
            // Tints the notification card using a color extracted from the
            // large icon (artwork) on Android 12+, matching the "dark card
            // with a hint of album-art color" look of the system media
            // player. ACCENT_COLOR is the fallback used pre-Android 12 or
            // when there's no artwork to sample from.
            .setColorized(true)
            .setColor(ACCENT_COLOR)
            .addAction(new NotificationCompat.Action(R.drawable.ic_notif_previous, "Previous",
                MediaButtonReceiver.buildMediaButtonPendingIntent(this, PlaybackStateCompat.ACTION_SKIP_TO_PREVIOUS)))
            .addAction(playing
                ? new NotificationCompat.Action(R.drawable.ic_notif_pause, "Pause",
                    MediaButtonReceiver.buildMediaButtonPendingIntent(this, PlaybackStateCompat.ACTION_PAUSE))
                : new NotificationCompat.Action(R.drawable.ic_notif_play, "Play",
                    MediaButtonReceiver.buildMediaButtonPendingIntent(this, PlaybackStateCompat.ACTION_PLAY)))
            .addAction(new NotificationCompat.Action(R.drawable.ic_notif_next, "Next",
                MediaButtonReceiver.buildMediaButtonPendingIntent(this, PlaybackStateCompat.ACTION_SKIP_TO_NEXT)))
            .addAction(new NotificationCompat.Action(R.drawable.ic_notif_close, "Close", closePI))
